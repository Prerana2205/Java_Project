...
public class InventoryManagementFrame extends JInternalFrame {
	...
	public InventoryManagementFrame() {
		...
		btnRemoveItem.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				if(textFieldRemoveItemID.getText().length() > 0) {
					inventory.removeItem(Integer.parseInt(textFieldRemoveItemID.getText()));
					textFieldRemoveItemID.setText("");
				}
				else {
		...
		btnPlaceOrder.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent arg0) {
				if(textFieldOrderQuantity.getText().length() > 0 && textFieldOrderItemID.getText().length() >0  ) {
					int quantity = Integer.parseInt(textFieldOrderQuantity.getText());
					if(quantity > 0) {
						inventory.placeOrder(Integer.parseInt(textFieldOrderItemID.getText()), Integer.parseInt(textFieldOrderQuantity.getText()));
						textFieldOrderItemID.setText("");
						textFieldOrderQuantity.setText("");
					...
					JOptionPane.showMessageDialog(null, "Please enter ItemId and Quantity");
				}
			}
		});
